exports.config = {
    ...
    baseUrl: 'http://test-app.openhouseinsurance.com',
    //
    // Default timeout for all waitFor* commands.
    waitforTimeout: 10000,
    //
    // Timeouts to be used in pages methods, the timeout depends on how much it takes to load
    timeout: {
        XS: 1000,
        S: 5000,
        M: 10000,
...
// ],
    ...
    before: function (capabilities, specs) {
        ...
        browser.addCommand('waitForClick', async function (timeout = 10000, reverse = false) {
            await this.waitForClickable({timeout,reverse});
            await this.click();
        }, true);

        browser.addCommand('waitForDropped', async function ({ timeout = 10000, x, y }) {
            await this.waitForDisplayed({ timeout });
            await this.dragAndDrop({ x, y });
        }, true);

        browser.addCommand('setVal', async function (value, timeout = 10000, reverse = false) {
